...
export default async (req) => {
  try {
    ...
    if (method === "POST") {
      ...
      const events = await readEvents();
      const event = { id: body.id || `TG-${Date.now()}`, name:String(body.name||"").trim().toUpperCase(), date:body.date, time:body.time, timeEnd:body.timeEnd, location:String(body.location||"").trim().toUpperCase(), capacity:Number(body.capacity||0), price:Number(body.price||0), comment:String(body.comment||"").trim(), rules:String(body.rules||"").trim(), active: body.active !== false, status:String(body.status||"Programado") };
    ...
    if (method === "PUT") {
      ...
      events[i]={...events[i],...body,name:String(body.name||events[i].name||"").trim().toUpperCase(),location:String(body.location||events[i].location||"").trim().toUpperCase(),comment:String(body.comment??events[i].comment??"").trim(),rules:String(body.rules??events[i].rules??"").trim(),timeEnd:body.timeEnd || "",capacity:Number(body.capacity),price:Number(body.price),status:String(body.status||events[i].status||"Programado")}; await eventsStore().setJSON("events",events); return json(events[i]);
    }
    if (method === "DELETE") {
